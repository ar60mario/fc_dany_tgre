...
public class ProductoService {
    ...
    private final ProductoBO bo = new ProductoBO();
    
    public List<Producto2> getProductosActivos() throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        List<Producto2> productos = null;
        try{
            productos = bo.getProductosActivos();
            tx.commit();
        }catch(Exception ex){
            tx.rollback();
    ...
    public List<Producto2> getProductosByFiltroActivos(String filtro) throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        List<Producto2> productos = null;
        try{
            productos = bo.getProductosByFiltroActivos(filtro);
            tx.commit();
        }catch(Exception ex){
            tx.rollback();
            throw new Exception(ex);
    ...
    public void saveCliente(Cliente2 cliente) throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        try {
            new Cliente2BO().saveCliente(cliente);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
    ...
    public List<Cliente2> getAllClientes() throws Exception {
        ...
        Transaction tx = session.beginTransaction();
        try {
            clientes = new Cliente2BO().getAllClientes();
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
