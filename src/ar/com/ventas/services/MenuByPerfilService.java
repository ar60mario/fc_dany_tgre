...
public class MenuByPerfilService {
...
//    }
    public MenuByPerfil saveMenuByPerfil(MenuByPerfil perfil) throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        MenuByPerfil p2 = null;
        try {
            p2 = new MenuByPerfilBO().saveMenuByPerfil(perfil);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
    ...
    public List<MenuByPerfil> getMenuByPerfilByPerfil(Perfil perfil) throws Exception {
        List<MenuByPerfil> perfiles = null;
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        try {
            perfiles = new MenuByPerfilBO().getMenuByPerfilByPerfil(perfil);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
    ...
    public MenuByPerfil updateMenuByPerfil(MenuByPerfil perfil) throws Exception {
        ...
        MenuByPerfil p2 = null;
        try {
            p2 = new MenuByPerfilBO().updateMenuByPerfil(perfil);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
